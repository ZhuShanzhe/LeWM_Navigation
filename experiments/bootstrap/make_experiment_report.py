"""Create Chinese experiment report from measured artifacts, not guessed results."""
import csv,json,statistics
from pathlib import Path
root=Path('/root/autodl-tmp/lewm_research')
summary=json.loads((root/'reports/evaluation_summary.json').read_text())
by={r['run']:r for r in summary}
train=json.loads((root/'runs/train_pilot500/training_summary.json').read_text())
prov=json.loads((root/'reports/provenance.json').read_text())
tags=[f'pretrained_default_s{s}' for s in [42,43,44]]+[f'random_seeded_default_s{s}' for s in [42,43,44]]+['pretrained_long75_s42','random_long75_s42','pilot500_default_s42']
assert all(t in by for t in tags), 'Some required runs are incomplete'
for s in [42,43,44]:
 a=json.loads((root/f'runs/pretrained_default_s{s}/invocation.json').read_text())
 b=json.loads((root/f'runs/random_seeded_default_s{s}/invocation.json').read_text())
 assert (a['episodes'],a['start_steps'])==(b['episodes'],b['start_steps'])
a=json.loads((root/'runs/pretrained_long75_s42/invocation.json').read_text())
b=json.loads((root/'runs/random_long75_s42/invocation.json').read_text())
assert (a['episodes'],a['start_steps'])==(b['episodes'],b['start_steps'])
def table(tags):
 lines=['| 实验 | 成功数 / 任务数 | 成功率 | 目标时间偏移 / 执行步数上限 |',
        '|---|---:|---:|---:|']
 for tag in tags:
  r=by[tag]; budget=150 if r['goal_offset_steps']==75 else 50
  lines.append(f"| {tag} | {r['successes']} / {r['n']} | {r['success_rate']:.1f}% | {r['goal_offset_steps']} / {budget} |")
 return '\n'.join(lines)
p=[by[f'pretrained_default_s{s}']['success_rate'] for s in [42,43,44]]
r=[by[f'random_seeded_default_s{s}']['success_rate'] for s in [42,43,44]]
with (root/'runs/train_pilot500/metrics.csv').open() as f: raw=list(csv.DictReader(f))
fit=[row for row in raw if row.get('fit/loss')]
val=[row for row in raw if row.get('validate/loss_epoch')]
report=f"""# LeWorldModel 首轮实验报告
...
默认任务：预训练模型三种子平均 **{statistics.mean(p):.2f}%**，种子间样本标准差 **{statistics.stdev(p):.2f} 个百分点**；固定动作随机种子的随机策略平均 **{statistics.mean(r):.2f}%**，标准差 **{statistics.stdev(r):.2f} 个百分点**。这里是同一官方权重的三个“评估种子”，不是三个独立训练模型。每种方法共 150 个评估实例；同一评估种子的起点和目标已逐一核对相同。
...
"""
for tag in ['pretrained_default_s42','random_seeded_default_s42','pretrained_long75_s42','random_long75_s42']:
 x=by[tag]
 report+=f"""
- {tag}：初始已经落在成功半径内 {x['initially_within_radius_n']}/{x['n']}；排除这些起点后成功 {x['outside_initial_radius_successes']}/{x['outside_initial_radius_n']}；墙两侧任务成功 {x['opposite_wall_sides_successes']}/{x['opposite_wall_sides_n']}。
"""
report+=f"""
...
## 4. 500 步训练试跑
- 完成 {train['global_step']} 个优化器步，5 个受限 epoch，每个 epoch 100 个 batch，batch size 128，每 epoch 验证 8 个 batch。
- 约 64,000 个训练窗口呈现次数；这不是 64,000 条独立轨迹，也不代表完整遍历数据集。
- 用时 {train['elapsed_seconds']:.2f} 秒（包含训练初始化后的 fit、验证和保存开销）。
- 首个记录点 step={fit[0]['step']}：总损失 {float(fit[0]['fit/loss']):.4f}，预测损失 {float(fit[0]['fit/pred_loss']):.4f}，SIGReg {float(fit[0]['fit/sigreg_loss']):.4f}。
- 末个记录点 step={fit[-1]['step']}：总损失 {float(fit[-1]['fit/loss']):.4f}，预测损失 {float(fit[-1]['fit/pred_loss']):.4f}，SIGReg {float(fit[-1]['fit/sigreg_loss']):.4f}。
- 末次验证总损失 {train['metrics']['validate/loss_epoch']:.4f}，预测损失 {train['metrics']['validate/pred_loss_epoch']:.4f}。
...
## 5. 配置和兼容性
- 官方代码 SHA：{prov['git_commit']}。
- GPU：{prov['gpu']}，显存约 32 GiB；Python {prov['python']}；PyTorch {prov['versions']['torch']}，CUDA {prov['torch_cuda']}。
- 官方下载检查点按当前实现统计含 {prov['official_checkpoint_parameters']:,} 个参数；这是实际加载对象的总参数数，不强行等同于论文中的近似模型规模表述。
...
"""
(root/'reports/experiment_report.md').write_text(report)
print(json.dumps({'pretrained_default_mean':statistics.mean(p),'random_default_mean':statistics.mean(r),'rows':[{k:by[t][k] for k in ['run','successes','n','success_rate']} for t in tags]},indent=2))
