...
for j in protocol['jobs']:
 ...
 trace=np.load(p/'trace.npz');trace_audit=audit_executed_trace(trace,rows)
 ...
 records.append({'tag':tag,'trace_audit':trace_audit,'model':v,'n':len(rows),'case_sha256':meta['case_sha256'],'candidate_model_steps':tel['candidate_model_steps'],'encoder_images':tel['encoder_images'],'batch_elapsed_seconds_not_online_latency':metric['elapsed_seconds'],'rows':rows,'cases':expected})
if mode=='smoke':
