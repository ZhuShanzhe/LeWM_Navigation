...
lines+=['','## 解释边界与后续','',
 ...
 '', '尚需阶段审阅、补充训练种子、关键迁移基线、多布局训练和新拓扑验证；部分观测是否加入由证据决定。整轮研究尚未完成。前沿方法及旧结果参见 frontier_transfer_review.md 和根目录 reports/navigation_capability_merged.md。']
...
for path in [R/'reports/frontier_transfer_review.md',ROOT/'reports/navigation_capability_merged.md']:
 body=path.read_text().split('### 当前执行快照')[0].split('## 阶段机制审计记录')[0].rstrip()
