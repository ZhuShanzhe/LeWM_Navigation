...
models={};records={};audits={};identity={};config=None;legacy_normalization=[]
 ...
 for part,batches,n in [('validation',2,100),('test',6,300)]:
  for task,h in [('short150',25),('long_base',75)]:
   ...
   for b in range(batches):
    ...
    if 'normalization_sha256' in meta:
     assert meta['normalization_sha256']==norm_sha and meta['normalization_path']==str(R/'splits/normalization.json')
    else:
     assert seed==3072 and meta['normalization']=='strict training set only'
     recorded_job=next(x for x in state['jobs'] if x['tag']==jt)
     assert 'LEWM_NORMALIZATION_FILE' not in recorded_job['env']
     assert read(R/'splits/manifest.json')['frozen_files_sha256']['normalization.json']==norm_sha
     legacy_normalization.append(jt)
...
payload={'passed':True,'updated_unix':time.time(),'models':models,'results':results,'trace_audits':audits,
 'legacy_runtime_normalization_hash_missing_tags':legacy_normalization,
 'legacy_note':'Seed3072 original metadata lacks per-run normalization hash; frozen manifest hash/current file and recorded default invocation verified, not a reconstructed runtime hash.',
